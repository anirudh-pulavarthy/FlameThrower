...
async fn main() {
    let tileset = load_texture("res/background.png").await.unwrap();
    tileset.set_filter(FilterMode::Nearest);

    let tiled_map_json = load_string("src/map.json").await.unwrap();
    let tiled_map = tiled::load_map(&tiled_map_json, &[("res/background.png", tileset)], &[]).unwrap();
